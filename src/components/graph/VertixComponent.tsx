...
import {
    ...
    useBeginVerticesAnimationsCommandListener,
    useFinishVerticesAnimationsCommandListener,
    usePlayerMadeMoveEventListener,
    usePlayerTurnChangedListener,
    useVisibleCardSelectedEventListener,
    VisibleCardSelectedEvent,
} from '../../events/events'
...
export const VertixComponent = (props: VertixProps): ReactNode => {
    ...
    useBeginVerticesAnimationsCommandListener((event) => {
        //vertix scored
    ...
    return (
        <div className={classes.join(' ')} data-id={props.vertix.id} style={style} onPointerDown={onPointerDown}>
            {cardConfiguration ? (
                <CardComponent
                    selected={cardConfiguration.selected}
                    card={cardConfiguration.card}
                    ownerTurnOrder={cardConfiguration.ownerTurnOrder}
                ></CardComponent>
            ) : (
                <div
                    className="vertix-image"
                    style={{ backgroundImage: `url(${getBackGroundFromId(props.vertix.id)})` }}
                ></div>
            )}
        </div>
    )
